...
if (![WXApi isWXAppInstalled]){
    ...
    [self.payManager WeChatPayWithAppid:appid(AppID)
                               Noncestr:Noncestr (订单唯一随机字符串)
                            Packagename:Packagename(应用包名称)
                              Partnerid:Partnerid(商户编号)
                               Prepayid:Prepayid];(预支付订单信息)
}
...
- (void)WeChatPayWithNonceStr:(NSString *)nonceStr{
    ...
    //发起请求
    [manager POST:@"https://api.mch.weixin.qq.com/pay/unifiedorder" parameters:string progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
