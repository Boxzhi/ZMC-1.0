...
- (void)setUI {
    ...
    table.separatorInset = UIEdgeInsetsMake(0, 0, 0, 20);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.lzDelegate && [self.lzDelegate respondsToSelector:@selector(AAFoldButton:didSelectIndex:)]) {
        [self.lzDelegate AAFoldButton:self didSelectIndex:indexPath.row];
    }
...
- (void)tableClose {
//    self.aaStatusBlock(NO);//block需要谨慎使用
    
    if (self.lzDelegate && [self.lzDelegate respondsToSelector:@selector(AAFoldButton:isOpen:)]) {
        [self.lzDelegate AAFoldButton:self isOpen:NO];
    }
...
- (void)tableOpen {
//    self.aaStatusBlock(YES);
    
    if (self.lzDelegate && [self.lzDelegate respondsToSelector:@selector(AAFoldButton:isOpen:)]) {
        [self.lzDelegate AAFoldButton:self isOpen:YES];
    }
