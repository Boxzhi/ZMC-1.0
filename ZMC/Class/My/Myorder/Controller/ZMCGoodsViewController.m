...
@interface ZMCGoodsViewController ()<UITableViewDataSource,UITableViewDelegate,WeChatStylePlaceHolderDelegate>{
...
@implementation ZMCGoodsViewController
...
- (void)getOrderListInfo {
    ...
    [CommonHttpAPI getOrderListWithParameters:[CommonRequestModel getOrderListWithOrder_status:@"3" pageNO:ChangeNSIntegerToStr(self.currentPage) page_size:@"30"] success:^(NSURLSessionDataTask *task, id responseObject) {
...
//    }];
    ...
    if (orderList_model.result.data[section].order_status == 0 || orderList_model.result.data[section].order_status == 3) {
        button.hidden = YES;
        button1.hidden = YES;
    }
