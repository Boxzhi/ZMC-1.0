...
@interface ZMCCompleteViewController ()<UITableViewDelegate,UITableViewDataSource,WeChatStylePlaceHolderDelegate,UIAlertViewDelegate> {
...
@implementation ZMCCompleteViewController
...
- (void)getOrderListInfo {
    ...
    [CommonHttpAPI getOrderListWithParameters:[CommonRequestModel getOrderListWithOrder_status:@"5" pageNO:ChangeNSIntegerToStr(self.currentPage) page_size:@"30"] success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    button.layer.borderColor = StringMiddleColor.CGColor;
...
//    }];
    ...
    if (orderList_model.result.data[section].order_status == 5) {
//        button.hidden = YES;
        button1.hidden = YES;
    }
...
{
    ZMCLog(@"clickButtonAtIndex:%ld",(long)buttonIndex);
    if (buttonIndex == 1) {
