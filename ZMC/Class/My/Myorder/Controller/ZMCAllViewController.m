...
@interface ZMCAllViewController ()<UITableViewDelegate,UITableViewDataSource,WeChatStylePlaceHolderDelegate,UIAlertViewDelegate> {
...
- (void)getOrderListInfo {
    ...
    [CommonHttpAPI getOrderListWithParameters:[CommonRequestModel getOrderListWithOrder_status:@"" pageNO:ChangeNSIntegerToStr(self.currentPage) page_size:@"30"] success:^(NSURLSessionDataTask *task, id responseObject) {
...
//    [button1 addTarget:self action:@selector(clickToConfirm:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (orderList_model.result.data[section].order_status == 3) {
        
        button.hidden = YES;
        button1.hidden = YES;
    }else if (orderList_model.result.data[section].order_status == 4) {
        ...
        button1.hidden = YES;
    }else if (orderList_model.result.data[section].order_status == 5 || orderList_model.result.data[section].order_status == 0 ) {
        ...
        [[button rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            ...
            [CommonHttpAPI postReminderOrderWithEspecId:[NSString stringWithFormat:@"/%ld",(long)orderList_model.result.data[section].order_id] WithParameters:nil success:^(NSURLSessionDataTask *task, id responseObject) {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        if (alertView.tag == 2) {
            [CommonHttpAPI postCancelOrderWithEspecId:[NSString stringWithFormat:@"/%ld",(long)orderList_model.result.data[select_section].order_id] WithParameters:nil success:^(NSURLSessionDataTask *task, id responseObject) {
            ...
            [CommonHttpAPI postDeleteOrderWithEspecId:[NSString stringWithFormat:@"/%ld",(long)orderList_model.result.data[select_section].order_id] WithParameters:nil success:^(NSURLSessionDataTask *task, id responseObject) {
